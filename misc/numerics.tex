...
we have
\begin{align*}
U^* H U = \mat{\sigma^\xi \cdot \pa{-i\na + k - K_2} & T(x) e^{i\pa{K_2 - K_1}x} \\ T(x)^*e^{i\pa{K_1 - K_2}x} & \sigma^\xi \cdot \pa{-i\na + k -K_1}}
\end{align*}
From \eqref{eq:UmatU}, that we consider again, we want $K_2- K_1 = q_1$, so 
\begin{align*}
	T(x) e^{i(K_2-K_1)x} &= T(x) e^{iq_1 x} = T_1 +T_2 e^{i(q_1 - q_2)x} +T_3 e^{i(q_1 - q_3)x} \\
& = T_1 +T_2 e^{-ic_1^*x} +T_3 e^{-i\pa{c_1^*+c_2^*}x}
\end{align*}
where
\begin{multline*}
c_1^* := q_2 - q_1 = a_1^* = k_\theta \sqrt{3} \mat{1/2 \\ \sqrt{3}/2}, \\
c_2^* := q_3 - q_2 = -a_1^* + a_2^* = k_\theta \sqrt{3} \mat{-1 \\ 0}, \\
a_2^* = c_1^* + c_2^*
\end{multline*}
hence
\begin{align*}
...
\begin{align*}
	U^* \mat{\bbW^+ & 0 \\ 0 & \bbW^-} U &= \mat{\bbW^+ & 0 \\ 0 & \bbW^-}
\end{align*}
